...
export const nodeService = {
  ...
  async createNode(
    ...
    const createdNode = await nodeRepository.create({
      ...
      coordinateX: input.coordinate?.x ?? 0,
      coordinateY: input.coordinate?.y ?? 0,
      coordinateZ: input.coordinate?.z ?? 0,
      lineCoordinateX: input.line_coordinate?.x ?? 0,
      lineCoordinateY: input.line_coordinate?.y ?? 0,
      lineCoordinateZ: input.line_coordinate?.z ?? 0,
      rotateX: input.rotate_xyz?.x ?? 0,
      rotateY: input.rotate_xyz?.y ?? 0,
      rotateZ: input.rotate_xyz?.z ?? 0,
  ...
  async bulkUpsertTree(
    ...
    for (const nodeInput of nodes) {
      ...
      const nodeData = {
        id: nodeId,
        name: nodeInput.name,
        status: nodeInput.status ?? undefined,
        company: nodeInput.company ?? undefined,
        color: nodeInput.color ?? undefined,
        level: nodeLevel,
        isActive: nodeInput.is_active ?? undefined,
        icon: nodeInput.icon ?? undefined,
        description: nodeInput.description ?? undefined,
        coordinateX: nodeInput.coordinate?.x ?? 0,
        coordinateY: nodeInput.coordinate?.y ?? 0,
        coordinateZ: nodeInput.coordinate?.z ?? 0,
        lineCoordinateX: nodeInput.line_coordinate?.x ?? 0,
        lineCoordinateY: nodeInput.line_coordinate?.y ?? 0,
        lineCoordinateZ: nodeInput.line_coordinate?.z ?? 0,
        rotateX: nodeInput.rotate_xyz?.x ?? 0,
        rotateY: nodeInput.rotate_xyz?.y ?? 0,
        rotateZ: nodeInput.rotate_xyz?.z ?? 0,
        size: nodeInput.size ?? undefined,
        parentId: parentId,
        tags: nodeInput.tags ?? [],
